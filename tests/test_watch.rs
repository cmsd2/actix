...
struct WatchSubscriber(Arc<AtomicUsize>);

impl Actor for WatchSubscriber {
    type Context = Context<Self>;
}

impl actix::Handler<WatchEvent> for WatchSubscriber {
    type Result = ();

    fn handle(&mut self, we: WatchEvent, _ctx: &mut actix::Context<WatchSubscriber>) {
...
fn test_watcher_stopped_event() {
    ...
    let stoppeds = Arc::new(AtomicUsize::new(0));
    let stoppeds2 = Arc::clone(&stoppeds);
    ...
    System::run(move || {
        let subscriber =
            WatchSubscriber::create(|_ctx: &mut Context<WatchSubscriber>| {
                WatchSubscriber(stoppeds2)
            });
        
        let addr =
            actix::Watcher::start(subscriber.recipient(), move |_| MyActor(starts2, restarts2, messages2));
        addr.do_send(Die);
        addr.do_send(Die);
    ...
    assert_eq!(stoppeds.load(Ordering::Relaxed), 1);
}
